...
import { Wan22LoraListField } from "./Wan22LoraListField";
...
interface GenerateWan22I2VModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const GenerateWan22I2VModal = ({
  isOpen,
  onClose,
  plugin,
  projectId,
  ...
  useEffect(() => {
    ...
    console.log(
      "GenerateWan22I2VModal opened with initialParams:",
      initialParams
    );
      ...
      if (
        ...
        if (value !== undefined && value !== "") {
          ...
          if (isNaN(numValue)) {
            ...
            // Special validation for num_frames: must be 1 + X * 4
            if (paramName === "num_frames") {
              if ((numValue - 1) % 4 !== 0) {
                newErrors[paramName] =
                  "Must be 1 + X * 4 (e.g., 5, 9, 13, 17, 21, ..., 81, ..., 121)";
              }
            }
          }
        }
  ...
  return (
    <Modal
      ...
      title={
        regenerateFrameId
          ? `Regenerate Video ${regenerateFrameId} with ${plugin.name.toUpperCase()}`
          : `Generate Video with ${plugin.name.toUpperCase()}`
      }
      size="large"
    >
      <form onSubmit={handleSubmit} className="generate-form">
        ...
        <div className="form-actions">
          ...
          <button type="submit" className="btn btn-primary">
            {regenerateFrameId ? "Regenerate" : "Generate Video"}
          </button>
        </div>
      </form>
    </Modal>
